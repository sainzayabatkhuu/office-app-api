...
public class PrivilegeServiceImpl implements PrivilegeService {
    ...
    @Override
    public Optional<PrivilegeDTO> delete(Long id, PrivilegeDTO entity) {
        Optional<Privilege> existingPrivilegeOpt = privilegeRepository.findById(id);
        if (existingPrivilegeOpt.isPresent()) {
            privilegeRepository.deleteById(id);
            return Optional.of(privilegeDTOMapper.apply(existingPrivilegeOpt.get()));
        } else {
            return Optional.empty();
        }
    }
